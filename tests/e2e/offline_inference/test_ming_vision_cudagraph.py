...
class EncoderGraphTestWorkerExtension:
    ...
    def encoder_graph_test_stats(self):
        ...
        if manager is None:
            return {"num_budgets": 0, "graph_hits": 0, "graph_misses": 0}
        return manager.get_cumulative_stats()
        ...
        try:
            ...
            for stage_stats in stats:
                ...
                for rank_stats in stage_stats:
                    assert bool(rank_stats["num_budgets"]) == enabled
                    if enabled:
                        assert rank_stats["graph_hits"] >= 2, "requests did not exercise encoder graph replay"
