...
def test_pipeline_reuses_the_minicpmo45_stages() -> None:
    ...
    assert venus.duplex_plugin == minicpmo.duplex_plugin
...
def test_renamed_talker_config_keeps_the_minicpmo45_context_policy() -> None:
    def policy(tts_model_type: str) -> tuple[int, int, bool]:
        tts_config = SimpleNamespace(model_type=tts_model_type, max_position_embeddings=4096)
        model_config = SimpleNamespace(
            max_model_len=8192,
            hf_config_name="tts_config",
            hf_config=SimpleNamespace(tts_config=tts_config),
        )
        return _resolve_talker_streaming_prompt_config(model_config)

    assert policy("realtime_venus_omni_tts") == policy("minicpmtts")
    assert policy("realtime_venus_omni_tts")[0] == 4096
