...
# CI collects tests/model_executor with "core_model and cpu" and
# "core_model and cuda". The gate tests below need no device, the capture and
# replay tests do, so the level mark is module-wide and the hardware marks are
# per test.
...
def test_disabled_by_default_without_config():
    ...
    assert not cfm._estimator_graph_enabled
    x, *_ = _make_inputs("cpu", 40)
    assert not cfm._estimator_graph_usable(x)


@pytest.mark.cpu
def test_config_disable_skips_graphs():
    cfm = _make_cfm("cpu", enabled=False)
    x, *_ = _make_inputs("cpu", 40)
    assert not cfm._estimator_graph_usable(x)
...
def test_config_overrides_are_read():
    cfm = _make_cfm("cpu", max_graphs=7)
    assert cfm._estimator_graph_max == 7
...
class TestCudaGraphEquivalence:
    ...
    def test_full_cache_retires_whole_generation(self):
        """Never evict one graph while peers stay live (issue #6457)."""
    ...
    def test_low_free_memory_stays_eager(self, monkeypatch):
        ...
        graphed, eager = self._gpu_pair()
        monkeypatch.setattr(current_omni_platform, "get_device_memory", lambda *_a, **_k: (0, 1 << 40))
        inputs = _make_inputs("cuda", 254)
        out = graphed.solve_euler(*inputs)
