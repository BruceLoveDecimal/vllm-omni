...
def test_absent_masks_are_not_expanded_into_all_ones():
    """``None`` means "every token is real" and must stay that way.

    Under sequence parallelism the caller passes ``None`` because a
    full-sequence mask no longer lines up with this rank's shard. Materialising
    an all-ones mask here would contradict the enclosing block, which skips its
    own masking for exactly that reason.
    """
    from vllm_omni.diffusion.models.mage_flow import mage_flow_layers

    class _DenseAttentionRecorder(nn.Module):
        def __init__(self):
            super().__init__()
            self.query_lengths = []
            self.masks = []
    ...
    _initialize_parameters(attention)
    recorder = _DenseAttentionRecorder()
    attention.attention = recorder
    ...
    # One dense call over the whole joint sequence, and no mask reaches the
    # backend: absent masks must not be materialised into an all-ones mask.
    assert recorder.query_lengths == [5]
    assert recorder.masks == [None]
    assert image_output.shape[:2] == (1, 3)
    assert text_output.shape[:2] == (1, 2)
...
def test_sp_rejects_request_level_image_padding():
    ...
    this is the matching runtime guard.
    """
