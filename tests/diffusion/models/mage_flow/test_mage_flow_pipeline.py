...
def test_pad_token_sequences_rejects_mismatched_feature_dims() -> None:
    with pytest.raises(ValueError, match="feature dimension"):
        MageFlowPipeline._pad_token_sequences(
            [
                torch.zeros(1, 2, 3),
                torch.zeros(1, 2, 4),
            ]
...
def test_pad_token_sequences_rejects_unbatched_sequences() -> None:
    with pytest.raises(ValueError, match=r"\[1, S, D\]"):
        MageFlowPipeline._pad_token_sequences([torch.zeros(2, 3)])
