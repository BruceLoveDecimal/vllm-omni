...
def test_stepwise_camera_script_carries_pose_tail_across_chunks() -> None:
    module = _load_pipeline_module()
    pipeline = _pipeline(module)
    pipeline._ar_height = 16
    pipeline._ar_width = 16
    script = ((("w",), ("w",), ("w",)), (("d",), ("d",), ("d",)))
    state = _stepwise_state(num_frames=21, script=script)
    fake = _FakeARState(state.request_id)

    with pipeline.bind_ar_diffusion_state(state.request_id, fake):
        pipeline.prepare_encode(state)
        first_tail = state.extra["camera_tail"].poses.clone()
        while state.chunk_index == 0:
            noise = pipeline.denoise_step(None, states=[state])
            pipeline.step_scheduler(state, noise)
            if state.chunk_denoise_completed:
                pipeline.post_decode(state)
        second_tail = state.extra["camera_tail"].poses.clone()

    torch.testing.assert_close(second_tail[0, 2, 3], first_tail[0, 2, 3])
    assert second_tail[0, 0, 3] > first_tail[0, 0, 3]
...
def test_stepwise_requires_bound_ar_state() -> None:
    module = _load_pipeline_module()
    pipeline = _pipeline(module)
    state = _stepwise_state(num_frames=21)
...
def test_stepwise_transformer_failure_unbinds_pipeline_state() -> None:
    module = _load_pipeline_module()
    transformer = _RecordingTransformer(raise_on_call=2)
    pipeline = _pipeline(module, transformer=transformer)
    pipeline._ar_height = 16
