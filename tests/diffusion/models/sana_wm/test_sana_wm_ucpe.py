...
from vllm_omni.diffusion.models.sana_wm.ucpe import _slice_rope_for_cam, prepare_prope_fns
...
def test_slice_rope_for_cam_accepts_tp_local_rope() -> None:
    """RoPE already sliced to cam size is returned as-is (idempotent guard)."""
    token_count = 6
    # WAN RoPE built for head_dim=224 has 112 complex dims. The camera
    # head uses rope_dim=112, targeting 56 complex dims.
    full = torch.randn(token_count, 112, dtype=torch.complex64)
    sliced = _slice_rope_for_cam(full, rope_dim=112)
    assert sliced.shape == (token_count, 56)

    # Passing an already-sliced (56-dim) tensor must be idempotent.
    already_local = sliced.clone()
    assert torch.equal(
        _slice_rope_for_cam(already_local, rope_dim=112),
        already_local,
    )
