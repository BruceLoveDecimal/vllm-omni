...
def test_streaming_refiner_self_attention_slices_split_rope_for_tp(monkeypatch) -> None:
    import torch

    import vllm_omni.diffusion.models.ltx2.ltx2_transformer as ltx2_transformer
    from vllm_omni.diffusion.models.sana_wm.pipeline_sana_wm_two_stages import (
        _streaming_refiner_self_attention,
    )

    monkeypatch.setattr(ltx2_transformer, "get_tensor_model_parallel_world_size", lambda: 2)
    monkeypatch.setattr(ltx2_transformer, "get_tensor_model_parallel_rank", lambda: 0)
    ...
    # Full TP RoPE has 4 heads; the local rank has 2 heads. The streaming
    # refiner path must slice RoPE before applying split rotary.
    rope = (
        torch.ones(1, 4, hidden_states.shape[1], 2),
        torch.zeros(1, 4, hidden_states.shape[1], 2),
    )

    out = _streaming_refiner_self_attention(
        attn=FakeSelfAttention(),
        hidden_states=hidden_states,
        query_rotary_emb=rope,
        n_context_tokens=2,
    )
