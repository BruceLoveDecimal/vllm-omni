...
| Concurrency | up to `max_num_seqs` (default 8) requests per forward as rows of one padded DiT batch, split when rows times the longest row would exceed 2048 positions (long references are compute-bound and gain nothing from padding); requests only share a batch when their schedule matches; prefix caching and chunked prefill stay off on the encoder |
...
- Key flags: `enforce_eager` on the encoder stage (it walks the decoder
  ...
  graph per (batch, target, text, reference) length bucket and batches
  concurrent requests. `enable_prefix_caching` must stay off
...
| Batching across requests | supported: padded, masked rows of one DiT forward, split so rows times the longest row stays under 2048 positions; equal-length clips share one codec decode | `docs/design/feature/diffusion_continuous_batching.md` |
