...
- **Streaming stepwise** — see [Streaming video serving](#streaming-video-serving).
  One `WS /v1/realtime/video` session produces the whole rollout, one video
  chunk per AR block. Camera motion is fixed when the rollout starts;
  mid-session camera interaction is not wired yet.
...
- The public transport is the stepwise `WS /v1/realtime/video` path, which
  cannot take mid-session camera interaction yet.
