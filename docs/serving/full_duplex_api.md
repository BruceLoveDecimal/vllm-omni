...
and runs on the same stages and duplex runtime, so the MiniCPM-o session
contract applies unchanged. In-stream text injection and `<delegate>`
delegation are not served yet.
...
Audio output needs a reference voice; the checkpoint ships one:
...
python examples/online_serving/minicpmo/realtime_duplex_demo.py \
  ...
  --input-wav input_16k_mono.wav \
  --ref-audio ./Realtime-Venus-Omni/assets/HT_ref_audio.wav \
  --output-dir /tmp/realtime-venus-duplex
```

`Realtime-Venus-Audio` reports MiniCPM-o 4.5's `model_type` in its
`config.json`; serve it with the same deploy configuration.
