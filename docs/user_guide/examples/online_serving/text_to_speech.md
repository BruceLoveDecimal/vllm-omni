...
Concurrent requests are batched: the diffusion stage runs up to `max_num_seqs` requests (8 in `vllm_omni/deploy/auk.yaml`) as rows of one padded DiT forward, so the per-step cost is shared instead of serialised. `max_num_seqs` is the whole policy: every row is padded to the longest one and the batch is never split, so size it for the traffic. Short, reference-free requests gain the most from sharing a forward (5 s targets keep gaining up to eight rows); long voice-clone rows (5 s target plus a 6 s reference) are compute-bound at one row and only pay for padding, so lower `max_num_seqs` to 2-4 on the diffusion stage when most requests look like that. Only requests with the same schedule share a batch (`num_inference_steps`, `guidance_scale`, `sway`, `t_grid`; AuK-Flash pins all of them), each request keeps its own `seed`, and a request that fails validation gets its own error without failing the others in its batch. `request_batch_max_wait_ms` (10 ms by default) lets an idle engine hold the first request briefly to collect a burst.
