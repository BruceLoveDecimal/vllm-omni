...
class Attention(nn.Module):
    def __init__(
        ...
        self.omni_attention = None
        if processor.attn_backend == "omni":
    ...
    def attend(self, query, key, value, mask):
        if self.omni_attention is None:
        ...
        output = self.omni_attention(
            query.transpose(1, 2),
            key.transpose(1, 2),
            value.transpose(1, 2),
            # AuK supplies a key-padding mask broadcast across heads/queries.
            # Omni FlashAttention's varlen path consumes its compact 2D form.
            attn_metadata=AttentionMetadata(attn_mask=None if mask is None else mask[:, 0, 0, :]),
        )
...
class AttnProcessor:
    def __init__(
        ...
        if attn_backend not in ("torch", "omni"):
            raise ValueError(f"Unsupported AuK attention backend: {attn_backend}")
    ...
    def __call__(
        ...
        if self.attn_backend in ("torch", "omni"):
            ...
            if self.attn_mask_enabled and mask is not None:
                ...
                attn_mask = None
            x = attn.attend(query, key, value, attn_mask)
