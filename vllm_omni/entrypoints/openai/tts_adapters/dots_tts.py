...
"""dots.tts serving adapter (AR base LM + DiT flow-matching side path).

Three conditioning modes reach ``/v1/audio/speech``:

* zero-shot — no ``ref_audio``;
* reference audio only — ``ref_audio``, giving the DiT a CAM++ x-vector;
* voice clone — ``ref_audio`` + ``ref_text``, which additionally prefills
  ...
  encoder's KV cache (upstream "prompt prefill").
...
(``prompt_audio_plan``) rather than each doing their own arithmetic.
"""
...
#: ``sample_rate / samples_per_patch`` prompt tokens (~6.25/s for the
#: released checkpoints) plus one CAM++ and one AudioVAE encode on the
#: engine's critical path, and the CAM++ extractor crops to 10 s anyway.
_MAX_REF_AUDIO_SECONDS = 30.0


@register_tts_adapter
class DotsTTSAdapter(ARTTSAdapter):
    """dots.tts talker, deployed under the shared ``latent_generator`` stage.

    Detection is by architecture only: ``latent_generator`` belongs to
    VoxCPM2, and claiming it here would take that stage away from VoxCPM2
    deployments that declare no architecture. ``matches()`` tests the
    architecture before any stage key, and the lower ``detect_priority``
    puts this adapter ahead of VoxCPM2's stage-key match.
    """
    ...
    def validate(self, request: "OpenAICreateSpeechRequest") -> str | None:
        ...
        # Resolves an uploaded voice into request.ref_audio/ref_text in
        # place, so the build path only ever sees inline reference audio.
    ...
    def apply_sampling_overrides(
        ...
        Prompt patches and generated patches share the talker's per-request
        FM buffer. Without this the buffer would fill mid-utterance and the
        talker would cut the request short at an arbitrary point instead of
        the request being sized correctly up front.
        """
        import copy

        additional = (prompt or {}).get("additional_information") or {}
    ...
    async def warmup(self) -> None:
        ...
        streaming AudioVAE) allocates its per-request workspaces and pays
        every lazy CUDA/cuBLAS initialization on the first patch it
        produces. Doing that here moves it off the first real request's
        time-to-first-audio, and it needs a real request because the base
        LM only runs under a vLLM ``ForwardContext``.
        """
        server = self.ctx.server
        t0 = time.time()
        logger.info("Running warmup speech request for model_type=%s", self.name)
        # dots.tts ships no speaker presets: "default" is the zero-shot
        # path, which is also the cheapest warmup (no reference encode).
    ...
    def _audio_patch_geometry(self) -> tuple[int, int]:
        ...
        ``prod(downsample_rates)`` waveform samples (upstream
        ``model.py:802``).
        """
    ...
    def _get_tokenizer(self):
        """The checkpoint's Qwen2 tokenizer, loaded once per process."""
