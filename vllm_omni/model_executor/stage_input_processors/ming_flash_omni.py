...
def build_ming_talker_prompt_token_ids_for_info(
    *,
    ...
    Returns ``None`` when no tokenizer or explicit slot override is available.
    """
    explicit_ids = _coerce_int_list(
        additional_info.get("native_talker_prompt_token_ids", additional_info.get("prompt_token_ids"))
    )
    if explicit_ids is not None:
        return explicit_ids

    explicit_len = additional_info.get("native_talker_prompt_len")
    if explicit_len is not None:
        try:
            prompt_len = int(explicit_len)
        except (TypeError, ValueError):
            prompt_len = 0
        if prompt_len > 0:
            return [0] * prompt_len
    ...
    # Preset metadata resolved from the talker's sidecar (see
    # stamp_ming_talker_voice_meta) overrides the request fields so the
    # slot count includes the preset's prompt_wav frames.
...
def stamp_ming_talker_voice_meta(request_info: dict[str, Any], *, stage_client: Any | None) -> None:
    ...
    that include the preset's prompt_wav frames, whose length is only known
    after the talker's AudioVAE runs. The talker publishes those lengths in a
    sidecar at init; here we read them and stamp ``native_talker_*`` onto the
    request. Their presence is also the signal the talker uses to decide
    whether to inject the preset — if the sidecar is missing nothing is
    stamped and the talker safely falls back to no preset (matching slots).
    """
    ...
    from vllm_omni.model_executor.models.ming_flash_omni.talker_module import load_ming_voice_meta

    meta = load_ming_voice_meta(model_id).get(voice_name)
    if not meta:
        return
