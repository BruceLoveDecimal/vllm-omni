...
class MossReferenceEncoder:
    ...
    async def encode(
        ...
        *,
        ...
        # --- hot path: check the cache before resolving when we already have a
        # stable key (named voice always; anonymous ref only once its content
        # hash is known from a prior resolve). ---
        if voice_name:
            hot_key: tuple | None = self._make_cache_key(voice_name, created_at)
            flight_key = f"voice:{voice_name.lower()}:{created_at}"
        else:
            artifact_key = get_artifact_key(ref_str)
            hot_key = self._make_cache_key("ref:" + artifact_key, 0) if artifact_key else None
            # Flight key is the request-side reference (not the content hash),
            # so concurrent requests for the same ref_str also share the
            # resolve/download, not just the encode.
            flight_key = "ref:" + _sha1(ref_str)

        if hot_key is not None:
            cached = self._speaker_cache.get(hot_key)
            if cached is not None:
                return _clone_out(cached["codes"])
        ...
        if task is not None:
            return _clone_out(await asyncio.shield(task))
    ...
    async def _resolve_and_encode(
        ...
        if voice_name:
            ...
            artifact_key = get_artifact_key(ref_str)
            key_name = ("ref:" + artifact_key) if artifact_key else ("ref:" + _sha1(ref_str))
