...
def ming_voice_meta_path(model_id: str | None) -> str:
    """Deterministic path for the per-voice prompt-length sidecar.

    The talker worker (which runs the AudioVAE) writes this file at model
    init; the stage input processor — a different process that must size
    prompt-KV slots before the model runs — reads it. Keyed by model id so
    unrelated deployments don't collide; a key miss just degrades to "no
    preset" rather than a slot mismatch.
    """
    import hashlib
    import os
    import tempfile

    key = hashlib.sha1(str(model_id or "default").encode("utf-8")).hexdigest()[:16]
    return os.path.join(tempfile.gettempdir(), "vllm_omni_ming_voice_meta", f"{key}.json")


def write_ming_voice_meta(model_id: str | None, meta: dict) -> None:
    """Atomically persist ``{voice_name: {prompt_text, prompt_wav_len, spk_emb_count}}``."""
    import json
    import os

    path = ming_voice_meta_path(model_id)
    try:
        ...
        logger.warning("Failed to write Ming voice meta to %s: %s", path, exc)


def load_ming_voice_meta(model_id: str | None) -> dict:
    """Load the voice-prompt sidecar; returns ``{}`` when absent/unreadable."""
    import json

    try:
        with open(ming_voice_meta_path(model_id)) as f:
            return json.load(f)
    except Exception:
        return {}
        ...
        *,
        ...
        if randn_tensor is None:
            ...
            if tuple(randn_tensor.shape) != expected_randn_shape:
                raise ValueError(
                    f"randn_tensor shape must be {expected_randn_shape}, got {tuple(randn_tensor.shape)}"
                )
