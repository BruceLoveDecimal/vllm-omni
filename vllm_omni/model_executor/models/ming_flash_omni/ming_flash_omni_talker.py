...
class MingFlashOmniTalkerForConditionalGeneration(nn.Module, CustomProcessMixin):
    ...
    def _validate_native_hidden_states(self, hidden_states: torch.Tensor) -> None:
        if not isinstance(hidden_states, torch.Tensor):
            raise RuntimeError(
                "Ming native talker expected tensor hidden states, "
    ...
    def _validate_unique_pending_requests(self) -> None:
        ...
        if duplicates:
            raise RuntimeError(
                "Ming native talker duplicate scheduler rows: "
                f"request_ids={duplicates}"
            )
        ...
        **info_dict: Any,
        ...
        try:
            if is_prefill:
                ...
                max_steps = self._native_prefill_max_steps(info_dict, params)
    ...
    def _build_native_prefill_embeds(
        ...
        *,
        ...
        if text:
            ...
            segments = segment_and_normalize(text, max_length=params.max_text_length)
            if segments:
    ...
    def _native_prefill_max_steps(self, info_dict: dict[str, Any], params: _GenerationParams) -> int:
    ...
    def _resolve_voice(self, additional_info: dict[str, Any]) -> _VoiceContext:
        ...
        # runs, so the prefill embeds must not exceed them. A preset's
        # prompt_wav length is only known after the AudioVAE runs, so the stage
        # input processor sizes the slots from the sidecar the talker publishes
        # at init and signals it via ``native_talker_prompt_wav_len``. Inject
        # the preset only when that signal is present (non-native always
        # injects); otherwise fall back to no preset so slots still match.
        native_slots_reserved = additional_info.get("native_talker_prompt_wav_len") is not None
        if (
            voice_name
            and spk_emb is None
            and voice_name in self.voice_presets
            and native_slots_reserved
        ):
...
def _stop_decision_mask(stop_out: torch.Tensor) -> torch.Tensor:
    ...
    sums = rows.sum(dim=-1)
    prob_like = (rows >= 0).all(dim=-1) & (rows <= 1).all(dim=-1) & torch.isclose(
        sums,
